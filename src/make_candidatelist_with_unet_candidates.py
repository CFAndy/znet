...
def fillTPCandidateList(candidates, annotation, mergedTPCandidatesAndFPCandidates):
    ...
    print "candidate list len: ", len(candidates)
    for indexCan in tqdm(range(0, len(candidates))):
        ...
        if (overlapping(can, possibleAnnotations) == False):
            ...
            if (DistanceToTPTooClose(can, possibleAnnotations, DISTANCE) == False):
                print "FP2"
                mergedTPCandidatesAndFPCandidates = mergedTPCandidatesAndFPCandidates.append({'seriesuid': can.seriesuid,
                                                                                              'coordX': can.coordX,
                                                                                              'coordY': can.coordY,
                                                                                              'coordZ': can.coordZ,
    ...
    for indexAnn in tqdm(range(0, len(annotation))):
        ...
        if found == True:
            mergedTPCandidatesAndFPCandidates = mergedTPCandidatesAndFPCandidates.append({'seriesuid': ann.seriesuid,
                                                                                          'coordX': x,
                                                                                          'coordY': y,
                                                                                          'coordZ': z,
